import { AxiosInstance } from 'axios'
import { api } from './axios'
import { sendTelegram } from '../utils/utils'

export async function autenticaFrota(apiFrota: AxiosInstance) {
  // para gerar o token da apiFrota (https://portal.profrotas.com.br/#!/visualizar-chave-integracao), precisa do token da api padrão.
  //

  try {
    apiFrota.defaults.headers.common.Authorization = ''
    const { data } = await api.post('/api/apiToken/frota/2359')
    if (!data.token) {
      sendTelegram('Erro autenticaFrota, sem token!')
    }
    console.log('Bearer ' + data.token)
    apiFrota.defaults.headers.common.Authorization = 'Bearer ' + data.token
  } catch (error: any) {
    console.log('Catch ao autenticar apiFrota:' + error.message)
    console.log('error.response.data:', error.response.data)
    sendTelegram('Catch ao autenticar apiFrota:' + error.message)
    return {}
  }
}

export async function setAutenticacaoErrada(api: AxiosInstance) {
  const token =
    'eyJhbGciOiJIUzUxMiJ9..J0_BvvCjo4Xd0hbFqx147RaNQOFRDdAPqPGKsFdXOh2D2EJb2yI2QwmFE7sjx4_5w9cfLxkFRKAsvEZCZAx6kw'
  api.defaults.headers.common.Authorization = 'Bearer ' + token
}
export async function setTokenFixo(api: AxiosInstance) {
  const token =
    'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ok-KdaIn25ftj7FTJ9Jg5uudLeqxo8apZ651JFI9Ykc'
  api.defaults.headers.common.Authorization = 'Bearer ' + token
}
